...
const toErrorMessage = (value: unknown): string | null => {
	...
	if (value && typeof value === "object") {
		...
		if (
			objectValue.constraints &&
			typeof objectValue.constraints === "object"
		) {
			return toErrorMessage(
				Object.values(
					objectValue.constraints as Record<string, unknown>,
				),
			)
		}
	}
...
export const parseError = (error: unknown): ApiError => {
	...
	const errorCode =
		typeof rawCode === "string" ? rawCode : ERROR_CODES.INTERNAL_ERROR
	const errorMessage =
