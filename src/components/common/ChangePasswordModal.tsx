...
export function ChangePasswordModal({ onClose, onSuccess }: ChangePasswordModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (form.newPassword !== form.confirm) { setError("Xác nhận mật khẩu không khớp"); return; }
    setSaving(true);
    setError("");
    const res = await identityService.changePassword(form.currentPassword, form.newPassword);
    setSaving(false);
    if (res.success) {
      onSuccess();
    } else {
    ...
    <div className="user-modal">
      <div className="user-modal__box">
        ...
        <div className="user-modal__actions">
          <button className="user-modal__btn user-modal__btn--primary" onClick={handleSave} disabled={saving}>
            {saving ? "Đang lưu..." : "Đổi mật khẩu"}
          </button>
          <button className="user-modal__btn" onClick={onClose} disabled={saving}>
            Hủy
          </button>
        </div>
      </div>
