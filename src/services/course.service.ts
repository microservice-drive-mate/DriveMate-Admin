...
export const courseService = {
	...
	getById: withErrorHandling((id: string) =>
		apiService.get<ApiResponse<CourseResponse>>(`/admin/courses/${id}`),
	),

	getPublicById: withErrorHandling((id: string) =>
		apiService.get<ApiResponse<CourseResponse>>(`/courses/${id}`),
	),

	create: withErrorHandling((payload: CreateCoursePayload) =>
		apiService.post<ApiResponse<CourseResponse>>("/admin/courses", payload),
	),
	...
	archive: withErrorHandling((id: string) =>
		apiService.delete<ApiResponse<CourseResponse>>(`/admin/courses/${id}`),
	),
	...
	createSchedule: withErrorHandling(
		(courseId: string, payload: CreateSchedulePayload) =>
	...
	updateSchedule: withErrorHandling(
		(
			courseId: string,
			scheduleId: string,
	...
	updateLesson: withErrorHandling(
		(courseId: string, lessonId: string, payload: UpdateLessonPayload) =>
			apiService.patch<ApiResponse<CourseResponse>>(
	...
	assignInstructor: withErrorHandling(
		(courseId: string, payload: AddCourseInstructorPayload) =>
			apiService.post<ApiResponse<CourseResponse>>(
