	...
	return (
		<div className="add-ec__section">
			...
			<div className="add-ec__form-body">
				...
				>
					Tuỳ chọn: xác định số câu lấy từ từng topic. Tổng số câu
					phải bằng {totalQuestions}. Nếu để trống, câu hỏi sẽ được
					lấy ngẫu nhiên từ toàn bộ ngân hàng.
				</span>
				...
				{rows.map((row, index) => (
					<div key={index} className="add-ec__topic-row">
						<select
							value={row.topicId}
							onChange={(e) =>
								onUpdateRow(index, { topicId: e.target.value })
							}
							style={{ flex: 1 }}
						>
