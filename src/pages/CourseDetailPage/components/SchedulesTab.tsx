...
export function SchedulesTab({
	...
	useEffect(() => {
		...
		Promise.all(instructorIds.map((id) => userService.getById(id))).then(
			(results) => {
				const resolved: Instructor[] = []
				const map: Record<string, string> = {}
				results.forEach((res, i) => {
					const name = res.success
						? (res.data?.fullName ?? instructorIds[i])
						: instructorIds[i]
	...
	const handleDelete = async (schedule: CourseSchedule) => {
		if (
			!confirm(
				`Xóa lịch học ${DAY_OF_WEEK_LABELS[schedule.dayOfWeek]} ${schedule.startTime}–${schedule.endTime}?`,
			)
		)
			return
	...
	if (loading) {
		return (
			<div className="course-detail__section">
				<p className="course-detail__empty">Đang tải lịch học...</p>
			</div>
		)
	}
	...
	return (
		<div className="course-detail__section">
			...
			{error && (
				<div className="course-detail__action-error">{error}</div>
			)}
									...
									<tr key={s.id}>
										...
										<td>
											<span
												className={
													s.isActive
														? "course-detail__badge--active"
														: "course-detail__badge--inactive"
												}
											>
												{s.isActive
													? "Hoạt động"
