...
export function MetricsSection({
	...
	const metricsBelongToSelectedService =
		metrics.serviceId === selectedService.id
		...
		<section className="system-health-section">
			<div className="system-health-section__header">
				<div>
					<h2>Metrics</h2>
					<span>
						Prometheus metrics for /{selectedService.prefix}/metrics
					</span>
				</div>
				<button
					type="button"
			...
			{metrics.error && metricsBelongToSelectedService && (
				<div className="system-health-inline-error">
					Metrics error{metrics.status ? ` (${metrics.status})` : ""}:{" "}
					{metrics.error}
				</div>
			)}
			...
			{showMetricsSkeleton ? (
				...
				<div className="system-health-metrics-grid">
					{metrics.parsed.map((metric) => (
						...
						>
							<span>
								{IMPORTANT_METRIC_LABELS[metric.name] ??
									metric.name}
							</span>
			...
			<details
				className="system-health-raw"
				open={!!metrics.error && metricsBelongToSelectedService}
			>
				<summary>Raw Prometheus text</summary>
				<pre>
					{metricsBelongToSelectedService && metrics.raw
						? metrics.raw
						: "No raw metrics loaded."}
				</pre>
			</details>
		</section>
	)
