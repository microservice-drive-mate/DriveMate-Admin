...
export function DocumentsTab({ userId }: Props) {
	...
	const load = useCallback(() => userService.getDocuments(userId), [userId])
	const {
		data: docs,
		loading,
		error,
		refetch,
	} = useAsyncData<UserDocument[]>(load, {
		initialData: [],
		enabled: true,
	...
	const handleAdd = async () => {
		if (!form.file) {
			setFormError("Vui lòng tải lên tệp tài liệu")
			return
		}
		setSaving(true)
		setFormError("")
	...
	return (
		<div className="card-surface student-detail__chart-card">
			...
			{viewError && (
				<p style={{ color: "#f87171", fontSize: 13 }}>{viewError}</p>
			)}
