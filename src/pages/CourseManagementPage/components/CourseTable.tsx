...
export function CourseTable({
	...
	if (!courses.length) {
		return <div className="course-empty">{emptyMessage}</div>
	}
				...
				<tbody>
					{courses.map((course, index) => (
						<tr key={course.id}>
							...
							<td className="course-table__code">
								{course.courseCode || "—"}
							</td>
							<td className="course-table__name">
								{course.title}
							</td>
							<td>
							...
							<td className="course-table__fee">
								{formatFee(course.tuitionFee)}
							</td>
							<td>
								<span
									className={`course-pill ${STATUS_PILL[course.status]}`}
								>
