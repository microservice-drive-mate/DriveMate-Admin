...
const CARD_CONFIG: Record<DashboardCard['key'], { title: string; icon: string; iconBg: string }> = {
  ...
  completedExams:{ title: 'Bài Thi Hoàn Thành',   icon: '📄', iconBg: '#8b5cf6' },
};

function toStatCard(card: DashboardCard): AdminStatCard {
  const cfg = CARD_CONFIG[card.key];
  const pct = card.delta.percentage;
  const sign = card.delta.direction === 'up' ? '+' : card.delta.direction === 'down' ? '-' : '';
  return {
    title: cfg.title,
    ...
    changeType: card.delta.direction === 'up' ? 'positive' : card.delta.direction === 'down' ? 'negative' : undefined,
  };
}
...
export function DashboardPage() {
  ...
  const statCards: AdminStatCard[] = loading || !data
