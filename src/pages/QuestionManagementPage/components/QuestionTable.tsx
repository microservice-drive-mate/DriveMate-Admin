...
import { DIFFICULTY_LABELS, QUESTION_TYPE_LABELS } from "@/types/question.types"
				...
				<tbody>
					{questions.map((q) => (
						...
						>
							...
							<td>
								<LicenseCategoryBadges
									categories={q.licenseCategories}
								/>
							</td>
							<td className="q-table__topic">
								{topicMap[q.topicId] ?? q.topicId}
							</td>
							<td>
								<span
									className={`q-badge ${DIFFICULTY_CLASS[q.difficulty]}`}
								>
							...
							<td>
								{q.isDeleted ? (
									<span className="q-status q-status--deleted">
										Đã xóa
									</span>
								) : q.isActive ? (
									<span className="q-status q-status--active">
										Hoạt động
									</span>
								) : (
									<span className="q-status q-status--inactive">
										Tạm dừng
									</span>
								)}
							</td>
