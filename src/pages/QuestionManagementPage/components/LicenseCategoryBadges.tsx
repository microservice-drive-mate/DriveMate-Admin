...
export function LicenseCategoryBadges({
	categories,
}: LicenseCategoryBadgesProps) {
		...
		return (
			<div className="q-class-list" aria-label={`Hang: ${tooltipLabel}`}>
				{visibleCategories.map((category) => (
					<span key={category} className="q-class-badge">
						{category}
					</span>
				))}
			</div>
	...
	return (
		<div className="q-class-list q-class-list--compact">
			<span
				className="q-class-tooltip"
				tabIndex={0}
				aria-label={`Hang: ${tooltipLabel}`}
			>
				<span className="q-class-badge q-class-badge--more">
