...
export default function QuestionManagementPage() {
	...
	const loadQuestions = useCallback(
		({ page, pageSize, filters }: PaginatedLoaderParams<QuestionFilters>) =>
			questionService.list({
				...
				...(filters.criticalStatus === "critical"
					? { isCritical: true }
					: {}),
				...(filters.criticalStatus === "normal"
					? { isCritical: false }
					: {}),
	...
	return (
		<div className="q-management">
			...
			{questionsList.error && (
				<div className="q-error">{questionsList.error}</div>
			)}
			{deleteError && <div className="q-error">{deleteError}</div>}

			<FilterBar
				filters={questionsList.filters}
				topics={topics}
				onChange={questionsList.setFilters}
			/>
