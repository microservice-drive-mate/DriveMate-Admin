...
|  2  | Login                          | Web \+ Mobile | POST /auth/login                                                               |                                  | Trả access_token \+ refresh_token.                                                        |
...
| 5   | Giảng viên Thêm khóa học mới (component giảng viên phụ trách)                             | Web      | GET /admin/users?role=instructor PATCH /admin/identity-users/:id/role                         | **P2** GET /admin/instructors **P2** POST /admin/courses/:id/instructors **P2** DELETE /admin/courses/:id/instructors/:userId              | TODO Figma: component giảng viên chọn nhiều — cần bảng join course_instructors. |
...
### 1\. Identity Service \- /auth/\*, /admin/identity-users/\* (14 endpoints) {#1.-identity-service---/auth/_,-/admin/identity-users/_-(14-endpoints)}

| Method | Auth   | Endpoint                       | Access | Mô tả                                            |
| :----- | :----- | :----------------------------- | :----- | :----------------------------------------------- |
| POST   | Public | /auth/login                    | Tất cả | Đăng nhập, cấp JWT access_token \+ refresh_token |
| POST   | JWT    | /auth/logout                   | User   | Đăng xuất, vô hiệu hóa session Keycloak          |
| POST   | Public | /auth/refresh                  | Tất cả | Dùng refresh_token cấp lại access_token mới      |
...
### 2\. User Service — /users/\*, /admin/users/\* (8 endpoints) {#2.-user-service-—-/users/_,-/admin/users/_-(8-endpoints)}
...
### 3\. Exam Service — /exams/\*, /admin/exams/\* (14 endpoints) {#3.-exam-service-—-/exams/_,-/admin/exams/_-(14-endpoints)}
...
### 4\. Course Service — /courses/\*, /enrollments/\*, /admin/courses/\* (16 endpoints) {#4.-course-service-—-/courses/_,-/enrollments/_,-/admin/courses/\*-(16-endpoints)}
...
### 6\. Notification Service — /notifications/\*, /admin/academic-warnings/\* (5 endpoints) {#6.-notification-service-—-/notifications/_,-/admin/academic-warnings/_-(5-endpoints)}
...
### 7\. Analytics Service — /analytics/\*, /admin/analytics/\* (2 endpoints) {#7.-analytics-service-—-/analytics/_,-/admin/analytics/_-(2-endpoints)}
...
### 8\. Simulation Service — /simulation/\*, /practice2d/\* (10 endpoints) {#8.-simulation-service-—-/simulation/_,-/practice2d/_-(10-endpoints)}
...
### 9\. Media Service — /media/\*, /admin/media/\* (6 endpoints) {#9.-media-service-—-/media/_,-/admin/media/_-(6-endpoints)}
...
Component giảng viên cần chọn nhiều, nên có bảng join course_instructors trong Course Service DB, không dùng 1 field instructor_id đơn.
